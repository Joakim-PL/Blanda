...
def generate_html(source, sections, preserve_paths=True, outdir=None):
    ...
    csspath = path.relpath(path.join(outdir, "pycco._temp"), path.split(dest)[0])
...
def process(sources, preserve_paths=True, outdir=None, language=None,
    ...
    if sources:
        ...
        css = open(path.join(outdir, "pycco._temp"), "wb")
        ...
        if index:
            with open(path.join(outdir, "template.html"), "wb") as f:
...
def main():
    ...
    parser.add_argument('-i', '--generate_index', action='store_true',
                      help='Generate an template.html document with sitemap content')
